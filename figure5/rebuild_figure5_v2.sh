...
if [[ -z "${LT_DP_OVERRIDE:-}" || -z "${LT_FLOW_OVERRIDE:-}" ]]; then
    echo "ERROR: please set the v2 environment variables first:"
...
assert t.averted_10yr.max() < 1e8 and t.averted_10yr.min() > -1e5, \
    f"!! outlier: max={t.averted_10yr.max():.0f}, min={t.averted_10yr.min():.0f}"
...
if not dp.scenario.str.startswith("single_").any():
    print(f"  WARN: {dp_file} has no single scenario, falling back to results/diet_policy_arm.csv")
...
if os.path.isfile(prev):
    ...
    if len(big):
        print(f"  rows changing by >10% ({len(big)}):")
        print(big.to_string(index=False))
    else:
        print("  all rows change <10% between v2/v1")
PYEOF
...
echo "  previous-version backup: results/archive_f5_prev2/"
