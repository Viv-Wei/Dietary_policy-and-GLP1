...
B_TRIAL = {"T2D":{"FPG":0.146},"CKD":{"FPG":0.048}}
COV_BASE = 0.20
...
def shift_from_rr(cur, base, rr, span, n=1000):
    for sp in (span, span*0.75, span*0.5, span*0.25):
        ...
        raise
    if rr > rrs[-1]:
        print(f"[warning] inversion: RR {rr:.3f} exceeds grid upper bound {rrs[-1]:.3f} (span={sp}), using upper bound")
...
def main(a):
    ...
    print(a.iso3, a.outcome, "total deaths:", round(tot),
          "inflow:", {k:round(v,3) for k,v in push.items()}, "depth:", depth)
